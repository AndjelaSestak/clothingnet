    ...
    IConfiguration config, IHubContext<NotificationHub> hubContext) : BaseApiController
{
    ...
    //zahtev koji stize od stripe-a nakon uspesnog checkout-a
    [HttpPost("webhook")]
    public async Task<IActionResult> StripeWebhook()
    {
        //event koji mi stripe salje
        ...
        try
        {
            ...
            return Ok(); //event prihvacen i hendlovan od strane mog backend-a
        }
    ...
    private async Task HandlePaymentIntentSucceeded(PaymentIntent intent)
    {
        if (intent.Status == "succeeded") 
        {
            ...
            {
                //ovu notifikaciju slusamo na klientskoj strani kad se ona desi, mogu da uradim nesto
    ...
    private Event ConstructStripeEvent(string json)
    {
        try
        {
            //uporedjujem stripe signature i uporedjujem sa whsecret koji dobijem od stripe-a
